...
Two anatomical categorisations are available in the dataset, and the choice
between them is consequential enough to be stated explicitly. The first is
the \emph{super-class}, a functional label taking nine values: \emph{optic},
\emph{central}, \emph{sensory}, \emph{descending}, \emph{ascending},
\emph{motor}, \emph{endocrine}, \emph{visual projection} and
\emph{visual centrifugal}. The second is the \emph{anatomical group}, which
records the neuropils in which a neuron arborises and takes $628$ distinct
values: \code{ME} for a neuron confined to the medulla, \code{AL.MB\_CA}
for one spanning the antennal lobe and the mushroom body calyx, and so on.

Paired with the five levels, the two give metanode sets that differ by more
than an order of magnitude: $24$ metanodes at super-class granularity against
$879$ at group granularity on the shallow window, with median populations of
$511$ and $3$ neurons respectively. Figure~\ref{fig:metagraph} shows both.
...
The cost of the finer choice is a long tail of very small metanodes, with a
median of three neurons on the shallow window. The $\mathit{min\_count}$
threshold of Section~\ref{sec:micro-methods} removes patterns supported by
too few instances, but it does not prevent a category from being small to
begin with, and this is a genuine limitation of the abstraction.
Section~\ref{sec:results-granularity} repeats the entire analysis at
super-class granularity and reports what each choice does and does not
recover.
...
medians marked. The group-level metagraph has $879$ nodes and $42{,}867$
...
rather than by functional super-class. Since that choice conditions every
result in this chapter, and since the alternative is the one adopted in the
related work of Section~\ref{sec:related}, the entire search was repeated at
super-class granularity so that the two could be compared directly rather
than argued about.

\begin{table}[H]
\centering
\caption{The same search on window $\{1,2,3\}$ under the two
categorisations.}
\label{tab:granularity}
\begin{tabular}{lrr}
\toprule
 & super-class & anatomical group \\
\midrule
categories                & 9 & 628 \\
metanodes                 & 24 & 879 \\
metagraph edges           & 293 & 42,867 \\
median neurons / metanode & 511 & 3 \\
motifs found              & 29,091 & 31,184,800 \\
runtime                   & 6.4\,s & 14m39s \\
\bottomrule
\end{tabular}
\end{table}
...
population. The coarse analysis is also very cheap, finishing in six seconds
and yielding twenty-nine thousand patterns rather than thirty-one million,
which makes it far easier to inspect by hand.

What the coarse analysis loses is anatomical resolution, and this is visible
in the leading pattern of each run. At super-class granularity it reads
\[
...
project to optic ones. At group granularity the leading pattern reads
\[
[\code{ME\_L2},\ \code{ME.LO\_L2}] \rightarrow \code{LO\_L3} \rightarrow
[\code{LO.LOP\_L2},\ \code{ME.LOP\_L2}],
\]
which names a circuit: two medulla populations converge on the lobula, which
distributes to the lobula plate. The first statement is true and the second
is more specific; whether the difference matters depends on the question
being asked.

For the questions asked in this thesis it matters decisively. The category
\emph{central} contains $521$ distinct anatomical groups, and the antennal
lobe projection neurons, the Kenyon cells, the mushroom body output neurons
and the lateral horn all fall within it without exception. The sequence
reported in Section~\ref{sec:results-sweep}, in which projection neurons
broadcast onto Kenyon cells that then feed integrative output
neurons, therefore has all three of its stages inside a single super-class
node and cannot be observed at that granularity at all. The same applies to
the compression results of Section~\ref{sec:results-compression}, which
identify particular neuropils as bottlenecks, and to the recurrence result of
Section~\ref{sec:optic-recurrence}, which separates re-entry into the lobula
from relay through the lobula plate.

The two granularities are therefore complementary rather than competing. The
coarse one gives a cheap global picture and is directly comparable with
earlier work on this connectome; the fine one is required for any claim about
which structure does what. Reporting both, as this section does, costs six
seconds of computation and removes the need to defend the choice in the
abstract.
